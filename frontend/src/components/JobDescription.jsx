...
const JobDescription = () => {
  ...
  try {
    ...
    const res = await axios.post(`${api}/api/application/apply/${params.id}`);

    if (res.data.success) {
      dispatch(setSingleJobById(res.data.updatedJob));
      setIsApplied(true);
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        axios.defaults.withCredentials = true;
        const res = await axios.get(`${api}/api/job/${params.id}`);
        if (res.data.success) {
